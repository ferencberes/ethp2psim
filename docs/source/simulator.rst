...
#. Playing with the `use_node_weights` parameter you can decide whether to sample message sources with respect to node weights (defined by :class:`ethp2psim.network.NodeWeightGenerator`) or just sample them uniformly at random from the nodes of the P2P network. **We note that adversarial nodes** (stored within the :class:`ethp2psim.adversary.Adversary`) **never get sampled to be message sources!** 
...
.. autoclass:: ethp2psim.simulator.Simulator
...
.. autofunction:: ethp2psim.simulator.Simulator.run
...
.. autofunction:: ethp2psim.simulator.Simulator.node_contact_time_quantiles
   
How to evaluate a simulation?
-----------------------------

We designed the :class:`ethp2psim.simulator.Evaluator` class to gain meaningful insights related to the deanonymization power of the :class:`ethp2psim.adversary.Adversary` and the average message node coverage (fraction of nodes reached by a given :class:`ethp2psim.message.Message`).

Using the :class:`ethp2psim.simulator.Evaluator` is very straightforward:

#. First, you run your simulator
#. Then, feed it to the :class:`ethp2psim.simulator.Evaluator` and specify the estimator that you want to use for deanonymization.
