...
from components import Network, Message, BroadcastProtocol
...
def test_single_message():
    ...
    msg = Message(0)
    first_ratio = msg.process(protocol)
    assert (first_ratio - 2/3) < 0.0001
    second_ratio = msg.process(protocol)
    assert (second_ratio - 5/6) < 0.0001
    third_ratio = msg.process(protocol)
